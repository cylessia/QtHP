...
class QTime extends QAbstractObject {
    ...
    private function _isValid($h, $m, $s, $u){
        ...
        return (($h !== null && ctype_digit($h) && $h >= 0 && $h < 0x018) || $h == null)
            && (($m !== null && ctype_digit($m) && $m >= 0 && $m < 0x03C) || $m == null)
            && (($s !== null && ctype_digit($s) && $s >= 0 && $s < 0x03C) || $s == null)
            && (($u !== null && ctype_digit($u) && $u >= 0 && $u < 0x3E8) || $u == null)
        ;    
    }
    ...
    private function _fromFormat($time, $format){
        if(!is_string($time) || ($hour = strpos(($format = strtolower($format)), 'hh')) === false || ($min = strpos($format, 'ii')) === false || ($secs = strpos($format, 'ss')) === false || ($ms = strpos($format, 'uuu'))){
            throw new QTimeInvalidFormatException('Invalid time format');
        }
        $this->setHMS(substr($time, $hour, 2), substr($time, $min, 2), substr($time, $secs, 2), ($ms !== false ? substr($time, $ms, 4) : '0'));
    }
}
