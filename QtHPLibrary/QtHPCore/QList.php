...
class QList extends QVector {
    ...
    public function __construct($template){
        ...
        if(!class_exists($template)){
            if(in_array($template, array(self::Bool, self::Integer, self::String, self::Float, self::Map))){
                ...
                throw new QListException('Unable to find the class "' . $template. '" ');
            }
        } else {
            $this->_template = $template;
    ...
    public function append($value, $recursive = false){
        if($value instanceof QList && $value->_template == $this->_template){
            ...
            throw new QListException('Not a ' . $this->_template);
        }
        return $this;
    }
    ...
    public function count($value){
        return count(array_keys($this->_list, $value, true));
    }
    ...
    public function indexOf($value, $from = null){
        ...
        if($from === null){
            ...
            throw new QListException('Out of range');
        }
     ...
     * @throws QListException If $value is not a template type
     ...
     * For example, a QList of strings would have this type : QList<string>
     * @return string
     */
...
class QListException extends QException{}
?>